...
describe('Balance Transactions API', () => {
  ...
  describe('GET /api/transactions/matches/:id', () => {
    it('finds matching entries for an unbalanced transaction', async () => {
      ...
      const res = await request(app)
        .get(`/api/transactions/matches/${transaction1._id}`);
      
      expect(res.statusCode).toEqual(200);
      expect(res.body.success).toBe(true);
      expect(res.body.data).toHaveProperty('transactions');
      // Since transactions are created in tests and timing can affect results,
      // we don't assert exact match count but check structure
      if (res.body.data.transactions.length > 0) {
        const matchedTx = res.body.data.transactions[0];
        expect(matchedTx).toHaveProperty('_id');
        expect(matchedTx).toHaveProperty('entries');
        expect(matchedTx.entries[0]).toHaveProperty('type');
        expect(matchedTx.entries[0]).toHaveProperty('amount');
      }
    });
    ...
    it('returns 400 for balanced transactions', async () => {
      ...
      const res = await request(app)
        .get(`/api/transactions/matches/${transaction._id}`);
      
      expect(res.statusCode).toEqual(400);
      expect(res.body.success).toBe(false);
      expect(res.body.error).toBe('Transaction is already balanced');
    });
  });
}); 